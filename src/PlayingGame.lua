...
local function updatePlayAreaObjectState(seatPos, obj, entered)
...
local function cachedPlayAreaCounts(seatPos)
	local state=playAreaObjectState[seatPos] or syncPlayAreaObjectState(seatPos)
	local cardCount=0
	local skillCount=0
...
local function scheduleContainerEndRoundStateRefresh(container)
...
local function objectInPlayerCombatArea(objectGUID)
...
local function recordPlayerFameChange(playerIndex, amount)
	local player=turnOrder[playerIndex]
			...
			if turnOrder[gStates.turnNumber].mage==gStates.positionMageKnight[5] then
				...
				if rewardChecklistActive then
					...
					if gStates.gameScenario=="Druid Nights" and gStates.druidNightsSummon~=nil then
						...
						rewardText=joinLang({rewardText, count, "{en}. Gained ", tostring(crystalReward), "{en} Random Crystal(s) from incantation"})
						count=count+1 linefeed=true
					end
...
local function removeGladeDiscardHealButton(obj)
...
local timeBendingGUID="2eb8e2"
local timeBendingRecoveryPending=false
local function reclaimTimeBending(callback)
...
local playerPickedUpPos={}
local playerPickedUpHex=nil
locationAttacked=false
local assaultApproachOrigin=nil
local assaultTargetPosition=nil
local wallAssaultChoiceResult=nil
	...
	if mouseButton=="-1" then
		if legalPlayerCheck(player.color, tonumber(id:sub(18,18)))==true then
			...
			if id:sub(1,17)=="NightTactic6Claim" then
				local seatPos=tonumber(id:sub(18,18)) or math.ceil((getObjectFromGUID("e2af14").getPosition()[1]+78)/40)
...
local pursuitStunnedImageURL="https://steamusercontent-a.akamaihd.net/ugc/9820771160644960489/29318B61F2A30E0E941F355C3664303B0C95BC60/"
