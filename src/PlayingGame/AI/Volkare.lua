									...
									if primaryDistance<2.5 or pairDistance<2.5 then
										...
										if gStates.defeatedCities.amount==0 then
											...
											if #gStates.volkareAttacked>=1 then
												...
												UI.setAttribute("CoopAssaultMainTableText2", "text", "{en}The City is under attack. Players must skip their next turn and Defend. This tool randomly gives the amount of attackers chosen to each player. All must face Volkare")
		...
		for x, mageDetails in pairs(turnOrder) do
			if gStates.volkareAttacked[1]~=nil and mageDetails.mage==gStates.volkareAttacked[1].mage then
				if playerRetreat==true then
					...
					broadcastToAll(joinLang({translateWord[mageDetails.mage], "{en} gained {ru} получает {zh-cn}增加了{ko}의 획득:  {es} ganó {fr} a subi {pt-br} ganhou {de} gewonnen ", (math.ceil(gStates.currentRound/2)+1), "{en} wounds.{ru} раны.{zh-cn}张创伤卡{ko}장의 부상{es} heridas.{fr} blessures.{pt-br} ferimentos.{de} Wunden."}), positionToColor(x))
					if gStates.volkareAttacked[1].distance<1.5 and gStates.volkareMovementStepPending~=true then broadcastToAll(joinLang({translateWord[mageDetails.mage], "{en} needs to withdraw to a neighboring Safe space.{ru} должен перейти на соседнее безопасное место.{zh-cn}需要强制撤退到附近的安全地带{ko}: 인접한 안전한 칸으로 강제후퇴해야 합니다.{es} necesita retirarse a un espacio seguro vecino.{fr} doit se retirer dans un espace sûr voisin.{pt-br} precisa se retirar para um espaço seguro vizinho.{de} sich auf ein benachbartes sicheres Feld zurückziehen muss."}), positionToColor(x)) end
				else
					...
					if returnScenario and gStates.volkareRaisedCity==true then
						...
						broadcastToAll("{en}Volkare Raised the City{ru}Волкар добрался до города{zh-cn}沃里卡占领了城市{ko}볼케어가 도시를 정복했습니다.{es}Volkare Levantó la Ciudad{fr}Volkare a Elevé la Ville{pt-br}Volkare Elevou a Cidade{de}Volkare hat die Stadt gehoben", positionToColor(x))
