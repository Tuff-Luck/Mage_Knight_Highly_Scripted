...
function __onLoad_raw(saved_data)
	...
	UI.setAttribute("helpButtonRealText", "text", "{en}Setup{ru}Настройки{zh-tw}设  置{zh-cn}设  置{ko}설정{es}Preparación{fr}Préparation{pt-br}Preparação{de}Einrichtung")
	UI.setAttribute("ScoreButtonRealText", "text", "{en}Score{ru}Результат{zh-tw}得  分{zh-cn}得  分{ko}점수{es}Puntuación{fr}Score{pt-br}Pontuação{de}Ergebnis")
	UI.setAttribute("TableButtonRealText", "text", "{en}Switch Table{ru}Дизайн стола{zh-tw}开关表{zh-cn}开关表{ko}테이블 전환{es}Cambiar Mesa{fr}Table de commutation{pt-br}Mudar de mesa{de}Tabelle wechseln")
	UI.setAttribute("MonsterButtonRealText", "text", "{en}Switch Enemy{ru}Дизайн врагов{zh-tw}切换敌人{zh-cn}切换敌人{ko}적 토큰 전환{es}Cambiar Enemigo{fr}Changement d'ennemi{pt-br}Trocar o inimigo{de}Feind wechseln")
	...
	UI.setAttribute("GameReminderTitleText", "text", "{en}Scenario Info{ru}Информация о сценарии{zh-tw}剧本信息{zh-cn}剧本信息{ko}시나리오 정보{es}Información del escenario{fr}Informations sur le scénario{pt-br}Informação do cenário{de}Szenario-Infos")
    ...
    UI.setAttribute("ObjectRotatingText", "text", "{en}<b>30deg</b> Rotation will allow everthing to align properly.{ru}Поворот на <b>30 градусов</b> позволит всему работать правильно.{zh-tw}<b>30 度</b> 旋转将使所有东西正确对齐。{zh-cn}<b>30 度</b> 旋转将使所有东西正确对齐。{ko}편리한 정렬을 위해 <b> 30도 </b> 회전 세팅을 추천합니다.{es}La rotación de <b> 30 grados </b> permitirá que todo se alinee correctamente.{fr}<b>30 deg</b> La rotation permettra à tout de s'aligner correctement.{pt-br}Rotação de <b>30deg</b> permitirá que tudo se alinhe adequadamente.{de}Eine <b>30 Grad</b> Drehung ermöglicht es, alles richtig auszurichten.")
	UI.setAttribute("ScoreBoardTableTitleText1", "text", "{en}Mage Knight Current Score{ru}Текущий счет{zh-tw}魔法骑士当前得分{zh-cn}魔法骑士当前得分{ko}현재 점수{es}Puntuación Actual de Mage Knight{fr}Mage Knight Score Actuel{pt-br}Pontuação atual do Mage Knight{de}Mage Knight Aktueller Spielstand")
	UI.setAttribute("ScoreBoardTableTitleText2", "text", "{en}Scores are calculated as if the game has been completed{ru}Очки подсчитываются так, как будто игра уже завершена{zh-tw}此分数视为游戏结束而统计{zh-cn}此分数视为游戏结束而统计{ko}점수는 게임이 종료된 것으로 간주하여 계산합니다{es}Las puntuaciones se calculan como si el juego se ubiera completado.{fr}Les scores sont calculés comme si le jeu était terminé{pt-br}Pontuação é calculada como se o jogo tivesse sido completado{de}Der Punktestand wird so berechnet, als ob das Spiel abgeschlossen wäre.")
	...
	UI.setAttribute("KnowledgeScoreHeadingText", "text", "{en}Knowledge{ru}Мудрец{zh-tw}博古通今{zh-cn}博古通今{ko}지식{es}Conocimiento{fr}Connaissances{pt-br}Conhecimento{de}Wissen")
	UI.setAttribute("LootScoreHeadingText", "text", "{en}Loot{ru}Добыча{zh-tw}富可敌国{zh-cn}富可敌国{ko}전리품{es}Botín{fr}Butin{pt-br}Saque{de}Beute")
	UI.setAttribute("LeaderScoreHeadingText", "text", "{en}Leader{ru}Лидер{zh-tw}至高领袖{zh-cn}至高领袖{ko}지도자{es}Líder{fr}Chef{pt-br}Líder{de}Anführer")
	UI.setAttribute("ConquerorScoreHeadingText", "text", "{en}Conqueror{ru}Завоеватель{zh-tw}战无不胜{zh-cn}战无不胜{ko}정복자{es}Conquistador{fr}Conquérant{pt-br}Conquistador{de}Eroberer")
	UI.setAttribute("AdventurerScoreHeadingText", "text", "{en}Adventurer{ru}Искатель приключений{zh-tw}披荆斩棘{zh-cn}披荆斩棘{ko}모험가{es}Aventurero{fr}Aventurier{pt-br}Aventureiro{de}Abenteurer")
	UI.setAttribute("LiberatorScoreHeadingText", "text", "{en}Liberator{ru}Исследователь подземелий{zh-tw}解放邻邦{zh-cn}解放邻邦{ko}해방자{es}Libertador{fr}Libérateur{pt-br}Libertador{de}Befreier")
	UI.setAttribute("BeatingScoreHeadingText", "text", "{en}Beating{ru}Поражение{zh-tw}满身疮痍{zh-cn}满身疮痍{ko}패배자{es}Golpeando{fr}Battement{pt-br}Espancamento{de}Schlagen")
	UI.setAttribute("RelicScoreHeadingText", "text", "{en}Relic Hunter{ru}Охотник за древностями{zh-tw}圣器猎者{zh-cn}圣器猎者{ko}유물 사냥꾼{es}Cazador de Reliquias{fr}Chasseur de Reliques{pt-br}Caçador de Relíquia{de}Reliquienjäger")
	UI.setAttribute("RewardScoreHeadingText", "text", "{en}Faction Rewards{ru}Награды фракций{zh-tw}派系奖励{zh-cn}派系奖励{ko}세력 보상{es}Recompensas de Facción{fr}Récompenses de Faction{pt-br}Recompensa de Facção{de}Fraktionsbelohnungen")
	UI.setAttribute("CityScoreHeadingText", "text", "{en}Conquered Cities{ru}Захваченный город{zh-tw}被征服的城市{zh-cn}被征服的城市{ko}정복한 도시{es}Ciudades Conquistadas{fr}Villes Conquises{pt-br}Cidades Conquistadas{de}Eroberte Städte")
	UI.setAttribute("TezlaScoreHeadingText", "text", "{en}Faction Greatest Enemy{ru}Величайший враг фракций{zh-tw}派系的至高敌人{zh-cn}派系的至高敌人{ko}세력의 위대한 적{es}El Mayor Enemigo de la Facción{fr}Le Plus Grand Ennemi de la Faction{pt-br}Maior Inimigo da Facção{de}Größter Feind der Fraktion")
	UI.setAttribute("VolkareScoreHeadingText", "text", "{en}General Volkare{ru}Генерал Волкар{zh-tw}沃卡里将军{zh-cn}沃卡里将军{ko}볼케어 장군{es}General Volkare{fr}Général Volkare{pt-br}General Volkare{de}General Volkare")
	UI.setAttribute("EfficiencyScoreHeadingText", "text", "{en}Efficiency{ru}Эффектиность{zh-tw}效率{zh-cn}效率{ko}효율 점수{es}Eficiencia{fr}Efficacité{pt-br}Eficiência{de}Effizienz")
	UI.setAttribute("TotalScoreText", "text", "{en}Total{ru}Итого{zh-tw}合计{zh-cn}合计{ko}총합{es}Total{fr}Le Total{pt-br}Total{de}Gesamt")
	UI.setAttribute("EndGameButtonText", "text", "{en}Scenario End Achieved - No{ru}Конец сценария достигнут - Нет{zh-tw}声明结束剧本-否{zh-cn}声明结束剧本-否{ko}시나리오 종료 조건 충족 전{es}Escenario Fin Realizados - No{fr}Scénario Fin Atteint - Non{pt-br}Fim do Cenário Alcançado - Não{de}Szenarioziel Erreicht – Nein")
	...
	UI.setAttribute("WallAssaultChoiceTitle", "text", "{en}Wall Fortification{ru}Укрепление стеной{zh-tw}城墙防御{zh-cn}城墙防御{ko}성벽 요새화{es}Fortificación de Muralla{fr}Fortification du mur{pt-br}Fortificação de Muralha{de}Mauerbefestigung")
	...
	UI.setAttribute("WallAssaultChoiceYesText", "text", "{en}Crossed Wall{ru}Через стену{zh-tw}穿过城墙{zh-cn}穿过城墙{ko}성벽 통과{es}Cruzó Muralla{fr}Mur franchi{pt-br}Atravessou Muralha{de}Über Mauer")
	UI.setAttribute("WallAssaultChoiceNoText", "text", "{en}Open Side{ru}Без стены{zh-tw}无墙一侧{zh-cn}无墙一侧{ko}열린 쪽{es}Lado abierto{fr}Côté ouvert{pt-br}Lado aberto{de}Offene Seite")
	...
	UI.setAttribute("Mage1CompleteText", "text", "{en}Create Card Pool{ru}Создать пул карт{zh-tw}创建卡池{zh-cn}创建卡池{ko}카드 풀 생성{es}Crear Grupo de Cartas{fr}Créer un Pool de Cartes{pt-br}Criar Conjunto de Cartas{de}Kartenpool erstellen")
	UI.setAttribute("Mage2CompleteText", "text", "{en}Create Card Pool{ru}Создать пул карт{zh-tw}创建卡池{zh-cn}创建卡池{ko}카드 풀 생성{es}Crear Grupo de Cartas{fr}Créer un Pool de Cartes{pt-br}Criar Conjunto de Cartas{de}Kartenpool erstellen")
	UI.setAttribute("Mage3CompleteText", "text", "{en}Create Card Pool{ru}Создать пул карт{zh-tw}创建卡池{zh-cn}创建卡池{ko}카드 풀 생성{es}Crear Grupo de Cartas{fr}Créer un Pool de Cartes{pt-br}Criar Conjunto de Cartas{de}Kartenpool erstellen")
	UI.setAttribute("Mage4CompleteText", "text", "{en}Create Card Pool{ru}Создать пул карт{zh-tw}创建卡池{zh-cn}创建卡池{ko}카드 풀 생성{es}Crear Grupo de Cartas{fr}Créer un Pool de Cartes{pt-br}Criar Conjunto de Cartas{de}Kartenpool erstellen")
