...
function DisplayHelp(player, mouseButton, id)
	if mouseButton=="-1" then
		...
		if gStates.positionMageKnight[5]=="Volkare" then
			...
			gameReminderText=joinLang({gameReminderText, "\n", translateWord[volkareCombatLevel[gStates.volkareCombatLevel]], "{en} Volkare Combat Level{ru} Уровень битвы Волкара{zh-cn} - 沃卡里战斗等级{ko} 볼케어 전투 레벨{es} Nivel de Carrera Volkare{fr} Volkare Niveau de Combat{pt-br} Nível de Combate de Volkare{de} Volkare Kampfstufe"}) gameReminderHeight=gameReminderHeight+lineFeed
		end
