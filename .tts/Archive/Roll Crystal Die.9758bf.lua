...
function monitorDice(color,roll)
    function coroutine_monitorDice()
        repeat
            ...
            coroutine.yield(0)
        until allRest == true
        --[[ tragic hack to add 2nd roll]]
        if roll then
            for _, die in ipairs(spawnedDice) do
                die.roll()
            end
            monitorDice(color,false)
            return 1
        else
...
function tragicHack()
    local value = {{270.00, 0.00, 0.00}, {0.00, 90.00, 0.00}, {0.00, 270.00, 270.00}, {0.00, 180.00, 90.00}, {0.00, 270.00, 180.00}, {90.00, 180.00, 0.00}}
    local h = 2
    local xOff = {}
        xOff[1] = {Vector(0,h,0)}
        xOff[2] = {Vector(-1.1,h,0), Vector(1.1,h,0)}
        xOff[3] = {Vector(-2.2,h,0), Vector(0,h,0), Vector(2.2,h,0)}
        xOff[4] = {Vector(-3.3,h,0), Vector(-1.1,h,0),Vector(1.1,h,0), Vector(3.3,h,0)}

    for n, die in ipairs(spawnedDice) do
        local c = #spawnedDice
        local pos = self.getPosition() + xOff[c][n]
        local result = die.getValue()
        die.setLock(true)
        die.setScale(Vector(3,3,3))

        local posOff = math.floor((self.getBounds().size.z/2)+(die.getBounds().size.z/2) + 0.5)
        print(posOff)
        die.setPositionSmooth(pos+Vector(0,0,0+posOff))
        die.setRotationSmooth(value[result])
    end
end
